...
local layouts = {
    ...
    main_with_stack = function(args, config)
        ...
        local sub = r:from(main_width, 0, r.width - main_width, r.height):set_layout("rows", {1, nil})

        if secondary_count ~= 0 then
            local secondary_height = math.floor(secondary_ratio * sub.height)
            local secondary = sub:from(0, 0, sub.width, secondary_height):set_layout("rows", {secondary_count, secondary_count})
            local remaining = sub:from(0, secondary_height, sub.width, sub.height - secondary_height):set_layout("rows", {1, nil}):fill_last()
        end
    ...
    centred = function(args, config)
        ...
        if secondary_count ~= 0 and args.count > 3 then
            ...
            if args.count > 4 then
                ...
                local right = c:from(main_offset + main_width, 0, main_offset, c.height):set_layout("rows", {1, nil})
            end
        else
            local left = c:from(0, 0, main_offset, c.height):set_layout("rows", {1, nil})
            local right = c:from(main_offset + main_width, 0, main_offset, c.height):set_layout("rows", {1, nil})
        end
